...
def test_handover_requires_receiver_and_transfers_accountability(client, headers):
    ...
    assert client.post(f"/api/handovers/{handover_id}/accept", headers=headers).status_code == 403
    grace = login(client, "grace.mensah@nursing.test")
    accepted = client.post(f"/api/handovers/{handover_id}/accept", headers=grace)
    assert accepted.status_code == 200
