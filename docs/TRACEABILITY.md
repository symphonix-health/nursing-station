...
`NFR-NS-011` is guarded by `tests/test_api.py`, `tests/test_governance_artifacts.py`, and the metadata-library seed-manifest validator. The runtime stores the declared manifest and landed counts in `seed_runs`; seeded patients retain their data class and manifest ID; the governance route exposes both. The Phase 1 seeder is explicitly approved for the declared non-production use cases and is not represented as `symphonix-health-assurance` or a live clinical source.
